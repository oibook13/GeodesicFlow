...
# Simplified training script for testing - uses base conda only
...
if [ -z "${CONFIG_ENV_FILE}" ]; then
    export CONFIG_ENV_FILE="config/config_proposed_flux.env"
fi
...
if [ -z "${MAIN_PROCESS_PORT}" ]; then
    echo "Set custom env vars permanently in config/config.env:"
    printf "MAIN_PROCESS_PORT not set, defaulting to 29500.\n"
...
# Skip dependency updates in testing
export DISABLE_UPDATES=1

# Run the training script using base conda python
if [[ -z "${ACCELERATE_CONFIG_PATH}" ]]; then
    ACCELERATE_CONFIG_PATH="${HOME}/.cache/huggingface/accelerate/default_config.yaml"
fi

# Use the PYTHON environment variable if set, otherwise use base conda python
if [ -n "${PYTHON}" ]; then
    PYTHON_CMD="${PYTHON}"
else
...
if [ -f "${ACCELERATE_CONFIG_PATH}" ]; then
    ...
    ${PYTHON_CMD} -m accelerate.commands.launch --config_file="${ACCELERATE_CONFIG_PATH}" --main_process_port="${MAIN_PROCESS_PORT}" train_proposed.py
else
    echo "Accelerate config file not found: ${ACCELERATE_CONFIG_PATH}. Using values from config.env."
    ${PYTHON_CMD} -m accelerate.commands.launch ${ACCELERATE_EXTRA_ARGS} --main_process_port="${MAIN_PROCESS_PORT}" --mixed_precision="${MIXED_PRECISION}" --num_processes="${TRAINING_NUM_PROCESSES}" --num_machines="${TRAINING_NUM_MACHINES}" --dynamo_backend="${TRAINING_DYNAMO_BACKEND}" train_proposed.py
fi
