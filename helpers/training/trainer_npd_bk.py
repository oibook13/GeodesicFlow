...
class Trainer:
    ...
    def _generate_trajectory(self, pipeline, prompt, generator):
        ...
        height = self.config.height or pipeline.unet.config.sample_size * pipeline.vae_scale_factor
        width = self.config.width or pipeline.unet.config.sample_size * pipeline.vae_scale_factor
        ...
        # Define a callback to store latents at each step
        def record_trajectory_callback(step, timestep, latents_at_step):
        ...
        with torch.no_grad():
            pipeline(
                ...
                latents=latents,
                generator=generator,
                num_inference_steps=50, # A typical number of steps for validation
                callback_steps=1,
                callback=record_trajectory_callback,
    ...
    def train(self):
            ...
            while True:
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if self.state["global_step"] % self.config.checkpointing_steps == 0:
                        # Log iteration and loss to train_log.csv
                        if self.accelerator.is_main_process:
                            ...
                            # Append current step's data. NPD is logged at the end of training.
                            with open(log_file_path, "a") as f:
                                f.write(f"{self.state['global_step']},{round(avg_loss.item(), 4)},N/A\n")
        ...
        if self.accelerator.is_main_process:
            ...
            # [START MODIFICATION] Add NPD calculation for the final model
            # NPD is most relevant for rectified flow models
            if self.config.flow_matching and self.validation_prompts:
                logger.info("Computing Normalized Path Deviation (NPD) on the final model...")
                try:
                    # Use a copy of the EMA model for validation if available
                    temp_unet = unwrap_model(self.accelerator, self.unet) if self.unet is not None else None
                    temp_transformer = unwrap_model(self.accelerator, self.transformer) if self.transformer is not None else None
                    if self.config.use_ema and self.ema_model is not None:
                        self.ema_model.copy_to(temp_unet.parameters() if temp_unet is not None else temp_transformer.parameters())
                    
                    # Temporarily build a pipeline for generation
                    pipeline_args = {
                         "vae": self.vae or AutoencoderKL.from_pretrained(self.config.vae_path, subfolder="vae" if self.config.pretrained_vae_model_name_or_path is None else None),
                         "text_encoder": self.text_encoder_1, "tokenizer": self.tokenizer_1,
                         "text_encoder_2": self.text_encoder_2, "tokenizer_2": self.tokenizer_2,
                         "unet": temp_unet, "transformer": temp_transformer,
                    ...
                    npd_scores = []
                    # Use first 3 validation prompts for stable averaging
                    for prompt in self.validation_prompts[:3]:
                        generator = torch.Generator(device=self.accelerator.device).manual_seed(self.config.seed or 0)
                        trajectory = self._generate_trajectory(temp_pipeline, prompt, generator)
                        npd_score = self.compute_npd(trajectory)
                        npd_scores.append(npd_score)
                        logger.info(f"NPD for prompt '{prompt[:60]}...': {npd_score:.6f}")
                    
                    if npd_scores:
                        avg_npd = sum(npd_scores) / len(npd_scores)
                        logger.info(f"Final Average NPD: {avg_npd:.6f}")
                        self.accelerator.log({"final_avg_npd": avg_npd}, step=self.state["global_step"])

                        # [MODIFICATION] Append the final NPD result to train_log.csv
                        log_file_path = os.path.join(self.config.output_dir, "train_log.csv")
                        if os.path.exists(log_file_path):
                            with open(log_file_path, "a") as f:
                                # At this final stage, loss isn't applicable, but we log the final iteration and NPD.
                                f.write(f"{self.state['global_step']},N/A,{avg_npd:.6f}\n")
                                f.flush()
                    ...
                    import traceback
                    logger.error(f"Could not compute NPD due to an error: {e}\n{traceback.format_exc()}")
            # [END MODIFICATION]
