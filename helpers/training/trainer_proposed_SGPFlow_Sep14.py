            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        ...
                        # Check if SGP-Flow is enabled
                        if getattr(self.config, 'riemannian_sgp_flow_enabled', True):
                            # ------------------------------------------------------------------
                            # --- Stable Geodesic-Projection Flow (SGP-Flow) START ---
                            # ------------------------------------------------------------------
                            
                            # Hyperparameters from config
                            d0 = getattr(self.config, 'riemannian_blending_midpoint_distance', 1.5707) # Default: pi/2
                            tau = getattr(self.config, 'riemannian_blending_transition_smoothness', 0.8)
                            EPS = 1e-6 # Small value for numerical stability

                            original_shape = latents.shape
                            
                            # Ensure inputs are valid manifold points (normalized for sphere)
                            latents_on_sphere = F.normalize(latents.view(bsz, -1), p=2, dim=1)
                            noise_on_sphere = F.normalize(input_noise.view(bsz, -1), p=2, dim=1)
                            
                            # Reshape for manifold operations
                            x0 = latents_on_sphere
                            eps = noise_on_sphere
                            t = sigmas.view(-1, 1)

                            # --- SGP-Flow Step 1: Compute Geodesic Distance and Blending Weight alpha ---
                            dot_product = torch.sum(x0 * eps, dim=1, keepdim=True)
                            geodesic_dist = torch.acos(torch.clamp(dot_product, -1.0 + EPS, 1.0 - EPS))
                            alpha = torch.sigmoid((geodesic_dist - d0) / tau)

                            # --- SGP-Flow Step 2: Compute Geodesic and Projection-based Initial Velocities ---
                            # Geodesic Initial Velocity (v_geo via Logarithmic Map)
                            theta = geodesic_dist
                            # Clamp sin(theta) to avoid division by zero for points that are close
                            v_geo = (theta / torch.sin(theta).clamp(min=EPS)) * (eps - torch.cos(theta) * x0)
                            
                            # Projection-based Initial Velocity (v_proj)
                            v_ambient = eps - x0
                            # Project v_ambient onto the tangent space T_x0(M)
                            v_proj = v_ambient - torch.sum(v_ambient * x0, dim=1, keepdim=True) * x0
                            
                            # --- SGP-Flow Step 3: Create the Blended Initial Velocity ---
                            v_blend = alpha * v_geo + (1 - alpha) * v_proj
                            
                            # --- SGP-Flow Step 4: Compute the Path (z_t) and Target Velocity (u_t) ---
                            # The path z_t is the geodesic starting from x0 with the blended initial velocity
                            v_blend_norm = torch.norm(v_blend, p=2, dim=1, keepdim=True).clamp(min=EPS)
                            
                            # Path z_t (via Exponential Map)
                            zt_flat = torch.cos(t * v_blend_norm) * x0 + torch.sin(t * v_blend_norm) * (v_blend / v_blend_norm)
                            
                            # Target velocity u_t (via Parallel Transport)
                            ut_flat = -v_blend_norm * torch.sin(t * v_blend_norm) * x0 + v_blend_norm * torch.cos(t * v_blend_norm) * (v_blend / v_blend_norm)

                            # Reshape back to original tensor shape
                            noisy_latents = zt_flat.view(original_shape)
                            target = ut_flat.view(original_shape)
                            
                            # SGP-Flow does not use an explicit regularization loss term
                            reg_loss = None

                            # ----------------------------------------------------------------
                            # --- Stable Geodesic-Projection Flow (SGP-Flow) END ---
                            # ----------------------------------------------------------------
                    ...
                    if self.config.flow_matching:
                        # SGP-Flow and Euclidean Flow both use MSE for the matching term
                        ...
                        # SGP-Flow has no explicit regularization term
