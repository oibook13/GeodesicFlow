...
def compute_rf_weights_lognorm(t: torch.Tensor, location: float = 0.0, scale: float = 1.0) -> torch.Tensor:
    """
    Computes Rectified Flow weights according to equation 18: w_t^π = (t/(1-t)) * π(t)
    where π(t) is the logit-normal density.
    
    Args:
        t (torch.Tensor): Timesteps in range (0,1)
        location (float): Location parameter (m) of logit-normal distribution
        scale (float): Scale parameter (s) of logit-normal distribution
    
    Returns:
        torch.Tensor: Weights for each timestep
    """
    # Ensure t is in (0,1) to avoid numerical issues
    eps = torch.finfo(t.dtype).eps
    t = t.clamp(eps, 1 - eps)
    
    # Compute logit(t) = log(t/(1-t))
    logit_t = torch.log(t) - torch.log1p(-t)
    
    # Compute logit-normal density π(t)
    # π(t) = 1/(s√(2π)) * 1/(t(1-t)) * exp(-(logit(t) - m)^2 / (2s^2))
    exp_term = -((logit_t - location) ** 2) / (2 * scale ** 2)
    normalizer = scale * torch.sqrt(torch.tensor(2 * torch.pi, device=t.device))
    pi_t = torch.exp(exp_term) / (normalizer * t * (1 - t))
    
    # Compute final weight: w_t^π = (t/(1-t)) * π(t)
    weights = (t / (1 - t)) * pi_t
    
    return weights

def compute_mode_mapping(u: torch.Tensor, s: float) -> torch.Tensor:
    """
    Implements equation (20): f_mode(u; s) = 1 - u - s * (cos²(π/2 * u) - 1 + u)
    
    Args:
        u (torch.Tensor): Input values in [0,1]
        s (float): Scale parameter controlling bias, must be in [-1, 2/(π-2)]
    
    Returns:
        torch.Tensor: Mapped values
    """
    cos_term = torch.cos(torch.pi / 2 * u) ** 2
    return 1 - u - s * (cos_term - 1 + u)

def compute_mode_inverse(t: torch.Tensor, s: float, num_iters: int = 50) -> torch.Tensor:
    """
    Computes inverse of f_mode using Newton's method since analytical inverse is challenging
    
    Args:
        t (torch.Tensor): Target values in [0,1]
        s (float): Scale parameter
        num_iters (int): Number of Newton iterations
    
    Returns:
        torch.Tensor: Inverse mapped values
    """
    # Initialize guess in middle of [0,1]
    u = torch.full_like(t, 0.5)
    
    for _ in range(num_iters):
        # Compute f_mode(u)
        f_u = compute_mode_mapping(u, s)
        
        # Compute derivative of f_mode
        cos_term = torch.cos(torch.pi / 2 * u)
        sin_term = torch.sin(torch.pi / 2 * u)
        df_du = -1 - s * (-torch.pi * cos_term * sin_term + 1)
...
def compute_mode_density(t: torch.Tensor, s: float) -> torch.Tensor:
    """
    Computes the density π_mode(t; s) by calculating d/dt f_mode^(-1)(t)
    
    Args:
        t (torch.Tensor): Input values in [0,1]
        s (float): Scale parameter
    ...
    """
    # Get inverse mapping
    u = compute_mode_inverse(t, s)
    
    # Compute derivative of f_mode at u
    cos_term = torch.cos(torch.pi / 2 * u)
    sin_term = torch.sin(torch.pi / 2 * u)
    df_du = -1 - s * (-torch.pi * cos_term * sin_term + 1)
    
    # Density is inverse of derivative
    return 1 / df_du.abs()

def compute_rf_weights_modesample(t: torch.Tensor, s: float = 0.0) -> torch.Tensor:
    """
    Computes Rectified Flow weights w_t^π = (t/(1-t)) * π(t) using mode sampling
    
    Args:
        t (torch.Tensor): Timesteps in range (0,1)
        s (float): Scale parameter controlling bias towards midpoint (positive s)
                  or endpoints (negative s). Must be in [-1, 2/(π-2)]
    
    Returns:
        torch.Tensor: Weights for each timestep
    """
    # Validate s is in valid range
    max_s = 2 / (np.pi - 2)
    if not -1 <= s <= max_s:
        raise ValueError(f"s must be in [-1, {max_s:.3f}], got {s}")
    
    # Ensure t is in (0,1) to avoid numerical issues
    eps = torch.finfo(t.dtype).eps
    t = t.clamp(eps, 1 - eps)
    
    # Compute mode density
    pi_t = compute_mode_density(t, s)
...
def compute_cosmap_forward(u: torch.Tensor) -> torch.Tensor:
    """
    Implements equation (21): f(u) = 1 - 1/(tan(π/2 * u) + 1)
    
    Args:
        u (torch.Tensor): Input values in [0,1]
    
    Returns:
        torch.Tensor: Mapped values t in [0,1]
    """
    # Ensure numerical stability by clamping u away from 0 and 1
    eps = torch.finfo(u.dtype).eps
    u = u.clamp(eps, 1 - eps)
    
    # Compute tan(π/2 * u)
    tan_term = torch.tan(torch.pi / 2 * u)
    
    return 1 - 1 / (tan_term + 1)

def compute_cosmap_inverse(t: torch.Tensor) -> torch.Tensor:
    """
    Computes inverse of f(u) to get u from t
    f^(-1)(t) = 2/π * arctan(1/(1-t) - 1)
    
    Args:
        t (torch.Tensor): Target values in [0,1]
    
    Returns:
        torch.Tensor: Inverse mapped values u in [0,1]
    """
    # Ensure numerical stability
    eps = torch.finfo(t.dtype).eps
    t = t.clamp(eps, 1 - eps)
    
    return 2/torch.pi * torch.atan(1/(1-t) - 1)

def compute_cosmap_density(t: torch.Tensor) -> torch.Tensor:
    """
    Implements equation (22): π_CosMap(t) = 2/(π - 2πt + 2πt²)
    ...
    Returns:
        torch.Tensor: Density values
    """
    return 2 / (torch.pi * (1 - 2*t + 2*t**2))

def compute_rf_weights_cosmap(t: torch.Tensor) -> torch.Tensor:
    """
    Computes Rectified Flow weights w_t^π = (t/(1-t)) * π(t) using CosMap sampling
    ...
    # Compute CosMap density
    pi_t = compute_cosmap_density(t)
            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        ...
                        if self.config.strategy_reweighting == "lognorm":
                            loss_weight = compute_rf_weights_lognorm(timesteps/1000).view(-1, 1, 1, 1)
                            loss = loss_weight * loss
                        elif self.config.strategy_reweighting == "modesample":
                            loss_weight = compute_rf_weights_modesample(timesteps/1000).view(-1, 1, 1, 1)
                            loss = loss_weight * loss
                        elif self.config.strategy_reweighting == "cosmap":
                            loss_weight = compute_rf_weights_cosmap(timesteps/1000).view(-1, 1, 1, 1)
                            loss = loss_weight * loss
