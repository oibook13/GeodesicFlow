...
class Trainer:
    ...
    def parse_arguments(self, args=None, disable_accelerator: bool = False):
        ...
        # NOTE: To enable Riemannian Flow Matching, add the following argument to your config:
        # "rcfm_flow": true
            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        if getattr(self.config, 'rcfm_flow', True):
                            # This implements the Euclidean case of Riemannian Flow Matching (RCFM)
                            # from "Flow Matching on General Geometries" (Chen et al., 2024).
                            # This corresponds to a straight path from noise x0 to data x1,
                            # with a constant velocity vector field, often known as Rectified Flow.
                            # Paper reference: Section 3.2, "Euclidean geometry", Eq. 165-167.
                            logger.debug("Using Riemannian Conditional Flow Matching (Euclidean case).")

                            # Sample t ~ U[0, 1] for each item in the batch.
                            # This corresponds to the time variable in the ODE.
                            t = torch.rand(bsz, device=self.accelerator.device)
                            
                            # Path construction: x_t = (1-t) * x_0 + t * x_1
                            # Here, `input_noise` is x_0 and `latents` is x_1.
                            t_reshaped = t.view(-1, 1, 1, 1)
                            noisy_latents = (1 - t_reshaped) * input_noise.float() + t_reshaped * latents.float()
                            noisy_latents = noisy_latents.to(dtype=self.config.weight_dtype)

                            # The target velocity field for this straight path is constant: u_t = x_1 - x_0
                            target = latents - input_noise

                            # The model expects timesteps in the [0, 1000] range.
                            timesteps = t * 1000.0
                    ...
                    if not getattr(self.config, 'rcfm_flow', True):
                    ...
                    if self.config.flow_matching and not getattr(self.config, 'rcfm_flow', True):
                    ...
                    elif not self.config.flow_matching:
                    ...
                    # If using RCFM, `target` is already defined.
