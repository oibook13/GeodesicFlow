...
class Trainer:
    ...
    def init_unload_text_encoder(self):
        ...
        # [MODIFICATION] Keep references to text encoders on CPU instead of deleting them
        # This allows them to be reloaded for intermittent validation/NPD calculation.
        ...
        logger.info(
            f"After moving text encoders to CPU, we freed {abs(round(memory_saved, 2))} GB of VRAM."
        )
    ...
    def init_validations(self):
        ...
        self.validation = Validation(
            ...
            ema_model=self.ema_model,
            vae=self.vae,
            controlnet=self.config.controlnet if self.config.controlnet else None,
        )
    ...
    def compute_npd(self, trajectory_points: list):
        """
        Computes the Normalized Path Deviation (NPD) for a given trajectory.
        NPD measures the deviation of a learned trajectory from an ideal straight line.

        Args:
            trajectory_points (list): A list of tensors representing the latents at each
                                      step of the generation process, from y_0 to y_1.

        Returns:
            float: The computed NPD value.
        """
        if not trajectory_points or len(trajectory_points) < 2:
            logger.warning("Trajectory must have at least two points to compute NPD.")
            return 0.0

        # The trajectory starts with the initial noise (y_0) and ends with the final latent (y_1)
        y0 = trajectory_points[0].float()
        y1 = trajectory_points[-1].float()
        K = len(trajectory_points) - 1

        # Precompute the terminal distance ||y_1 - y_0||
        endpoint_dist = torch.linalg.norm(y1 - y0)

        # Avoid division by zero if endpoints are identical
        if endpoint_dist < 1e-9:
            return 0.0

        deviation_sum = 0.0
        # Process all discretization points from k=0 to K
        for k, y_tk in enumerate(trajectory_points):
            t = k / K
            # Calculate the position on the ideal straight path
            z_k = (1 - t) * y0 + t * y1
            # Accumulate the Euclidean deviation
            deviation_sum += torch.linalg.norm(y_tk.float() - z_k.float())

        # Normalize the total deviation
        npd = deviation_sum / ((K + 1) * endpoint_dist)
        return npd.item()

    def _generate_trajectory(self, pipeline, prompt, generator):
        """
        Generates an image for a prompt and captures the full generation trajectory
        by using the modern `callback_on_step_end` API.

        Args:
            pipeline: The diffusion pipeline to use for generation.
            prompt (str): The prompt for which to generate the trajectory.
            generator: A torch.Generator for reproducibility.

        Returns:
            list: A list of latent tensors representing the trajectory.
        """
        # Handle both UNet and Transformer based pipelines
        if hasattr(pipeline, "transformer"):
            model_component = pipeline.transformer
        elif hasattr(pipeline, "unet"):
            model_component = pipeline.unet
        else:
            raise ValueError("The provided pipeline has neither a 'transformer' nor a 'unet' attribute.")

        # Prepare initial latents so we can capture the starting point (y_0)
        height = getattr(self.config, 'height', None) or model_component.config.sample_size * pipeline.vae_scale_factor
        width = getattr(self.config, 'width', None) or model_component.config.sample_size * pipeline.vae_scale_factor
        num_channels_latents = model_component.config.in_channels

        latents = pipeline.prepare_latents(
            1, num_channels_latents, height, width, self.config.weight_dtype, self.accelerator.device, generator
        )

        trajectory_points = [latents.clone().cpu()]

        # Define the callback function with the correct signature for modern pipelines
        def record_trajectory_callback(pipe, step_index, timestep, callback_kwargs):
            latents_at_step = callback_kwargs["latents"]
            trajectory_points.append(latents_at_step.clone().cpu())
            return callback_kwargs

        with torch.no_grad():
            # [MODIFICATION] Use the correct, modern callback argument `callback_on_step_end`
            pipeline(
                prompt=prompt,
                latents=latents,
                generator=generator,
                num_inference_steps=50,
                callback_on_step_end=record_trajectory_callback,
                negative_prompt=getattr(self.config, 'negative_prompt', None),
            )

        return trajectory_points

    def _get_intermittent_npd_score(self):
        """
        Safely computes the NPD score during training. This is a complex operation
        that involves rebuilding an inference pipeline. It manages model and component
        device placement and EMA state.
        
        Returns:
            A float NPD score or "N/A" if the calculation fails.
        """
        if not self.accelerator.is_main_process or not self.config.flow_matching or not self.validation_prompts:
            return "N/A"

        logger.info(f"Computing intermittent NPD for step {self.state['global_step']}...")
        
        temp_pipeline = None
        text_encoder_1_device, text_encoder_2_device, text_encoder_3_device, vae_device = None, None, None, None
        
        # Determine the primary model for unwrapping and EMA application
        model_to_use = self.unet if self.unet is not None else self.transformer
        if model_to_use is None:
            logger.warning("No UNet or Transformer model found for NPD calculation.")
            return "N/A"
        unwrapped_model = unwrap_model(self.accelerator, model_to_use)

        try:
            # 1. Apply EMA weights if enabled for an accurate validation metric
            if self.config.use_ema and self.ema_model is not None:
                self.ema_model.store(unwrapped_model.parameters())
                self.ema_model.copy_to(unwrapped_model.parameters())

            # 2. Ensure pipeline components are loaded and on the correct device.
            
            # VAE may have been unloaded. Reload it if necessary.
            vae = self.vae
            if vae is None:
                logger.warning("Temporarily reloading VAE for NPD calculation.")
                vae = AutoencoderKL.from_pretrained(
                    self.config.vae_path, subfolder="vae" if self.config.pretrained_vae_model_name_or_path is None else None
                )
            
            # Move all components to the accelerator's device
            vae_device = vae.device
            # [FIX] Cast VAE to the same dtype as the latents/model to prevent mismatch error
            vae.to(self.accelerator.device, dtype=self.config.weight_dtype)

            if self.text_encoder_1:
                text_encoder_1_device = self.text_encoder_1.device
                self.text_encoder_1.to(self.accelerator.device)
            ...
            if self.text_encoder_3:
                text_encoder_3_device = self.text_encoder_3.device
                self.text_encoder_3.to(self.accelerator.device)

            # 3. Build the inference pipeline
            pipeline_args = {
                "vae": vae, "text_encoder": self.text_encoder_1, "tokenizer": self.tokenizer_1,
                "text_encoder_2": self.text_encoder_2, "tokenizer_2": self.tokenizer_2,
                "scheduler": UniPCMultistepScheduler.from_config(self.noise_scheduler.config)
            }
            if self.unet is not None: pipeline_args["unet"] = unwrapped_model
            if self.transformer is not None: pipeline_args["transformer"] = unwrapped_model

            if self.config.model_family == "sd3":
                pipeline_args["text_encoder_3"] = self.text_encoder_3
                pipeline_args["tokenizer_3"] = self.tokenizer_3
                temp_pipeline = StableDiffusion3Pipeline.from_pretrained(self.config.pretrained_model_name_or_path, **pipeline_args)
            elif self.config.model_family == "flux":
                from diffusers.pipelines import FluxPipeline
                temp_pipeline = FluxPipeline.from_pretrained(self.config.pretrained_model_name_or_path, **pipeline_args)
            else:
                temp_pipeline = StableDiffusionXLPipeline.from_pretrained(self.config.pretrained_model_name_or_path, **pipeline_args)
            
            # 4. Generate trajectory and compute NPD
            generator = torch.Generator(device=self.accelerator.device).manual_seed(self.config.seed or 0)
            prompt_for_npd = self.validation_prompts[0]
            trajectory = self._generate_trajectory(temp_pipeline, prompt_for_npd, generator)
            npd_score = self.compute_npd(trajectory)
            
            logger.info(f"Intermittent NPD Score: {npd_score:.6f}")
            self.accelerator.log({"npd_score": npd_score}, step=self.state["global_step"])
            return round(npd_score, 6)

        except Exception as e:
            import traceback
            logger.error(f"Could not compute intermittent NPD score due to an error: {e}\n{traceback.format_exc()}")
            return "N/A"

        finally:
            # 5. Restore state to allow training to continue seamlessly
            if self.config.use_ema and self.ema_model is not None:
                self.ema_model.restore(unwrapped_model.parameters())

            # Move components back to their original devices
            if self.text_encoder_1 and text_encoder_1_device: self.text_encoder_1.to(text_encoder_1_device)
            if self.text_encoder_2 and text_encoder_2_device: self.text_encoder_2.to(text_encoder_2_device)
            if self.text_encoder_3 and text_encoder_3_device: self.text_encoder_3.to(text_encoder_3_device)
            if 'vae' in locals() and vae_device: vae.to(vae_device)
            ...
            while True:
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if self.state["global_step"] % self.config.checkpointing_steps == 0:
                        # [MODIFICATION] Get intermittent NPD score when checkpointing
                        npd_score = self._get_intermittent_npd_score()
                        if self.accelerator.is_main_process:
                            ...
                            if not os.path.exists(log_file_path):
                                with open(log_file_path, "w") as f:
                                    f.write("iteration,loss,npd\n")
                            
                            # Append current step's data.
                            with open(log_file_path, "a") as f:
                                f.write(f"{self.state['global_step']},{round(avg_loss.item(), 4)},{npd_score}\n")
