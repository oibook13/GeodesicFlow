...
training_logger_level = os.environ.get("SIMPLETUNER_TRAINING_LOOP_LEVEL", "INFO")
...
# --- Start of Riemannian Flow Implementation ---
class Manifold:
    """
    Abstract base class for manifold operations, providing the interface for
    Riemannian geometry calculations as required by the RCFM method.
    """
    def __init__(self, name='euclidean'):
        self.name = name

    def log(self, x0, x1):
        """Computes the logarithmic map from point x0 to x1."""
        raise NotImplementedError

    def exp(self, x0, v):
        """Computes the exponential map from point x0 with tangent vector v."""
        raise NotImplementedError

    def parallel_transport(self, x0, x1, t, v0):
        """Performs parallel transport of vector v0 along the geodesic from x0 to x1."""
        raise NotImplementedError

    def norm_sq(self, x, v):
        """Computes the squared norm of a tangent vector v at point x, induced by the metric."""
        raise NotImplementedError


class EuclideanManifold(Manifold):
    """
    Implementation of manifold operations for a standard Euclidean space.
    This is used for the latent space of the baseline model.
    """
    def __init__(self):
        super().__init__('euclidean')

    def log(self, x0, x1):
        """Logarithmic map in Euclidean space: log_{x0}(x1) = x1 - x0."""
        return x1 - x0

    def exp(self, x0, v):
        """Exponential map in Euclidean space: exp_{x0}(v) = x0 + v."""
        return x0 + v

    def parallel_transport(self, x0, x1, t, v0):
        """
        Parallel transport along a straight line (geodesic) in Euclidean space
        is the identity operation. The vector does not change.
        """
        return v0

    def norm_sq(self, x, v):
        """
        The squared norm induced by the Euclidean metric is the sum of squared components,
        which is equivalent to the mean squared error over pixels when averaged.
        """
        return torch.sum(v ** 2, dim=list(range(1, len(v.shape))))


def get_manifold(name='euclidean'):
    """Factory function to retrieve a manifold object by name."""
    if name.lower() == 'euclidean':
        return EuclideanManifold()
    # Future manifolds like 'sphere' or 'poincare' could be added here.
    # elif name.lower() == 'sphere':
    #     return SphereManifold()
    else:
        raise ValueError(f"Unknown manifold: {name}")

# --- End of Riemannian Flow Implementation ---
...
class Trainer:
    ...
    def run(self):
        try:
            ...
            # --- Initialize Riemannian Manifold for RCFM ---
            if getattr(self.config, 'riemannian_flow', True):
                manifold_name = getattr(self.config, 'riemannian_manifold', 'euclidean')
                self.manifold = get_manifold(manifold_name)
                logger.info(f"Initialized Riemannian Flow with '{manifold_name}' manifold.")
            # ---
        ...
        for epoch in range(self.state["first_epoch"], self.config.num_train_epochs + 1):
            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    # --- Start of Riemannian Flow Implementation ---
                    # Use Riemannian Flow by default, as requested.
                    # This section replaces the original flow_matching logic with the explicit RCFM formulation.
                    if getattr(self.config, 'riemannian_flow', True):
                        if self.manifold is None: # Lazy init if somehow missed in run()
                            self.manifold = get_manifold(getattr(self.config, 'riemannian_manifold', 'euclidean'))
                        
                        training_logger.debug("Using Riemannian Conditional Flow Matching (RCFM) objective.")
                        # In the context of the code:
                        # x0 (data point on manifold) is `latents`
                        # x1 (noise point on manifold) is `noise`
                        x0, x1 = latents, noise

                        # Sample time t, which corresponds to sigmas in the original implementation.
                        # We reuse the same sigma sampling strategy for consistency with the baseline.
                        ...
                        t_reshaped = sigmas.view(-1, 1, 1, 1)
                        timesteps = sigmas * 1000.0

                        # RCFM Step 1: Compute initial velocity v0 = log_{x0}(x1)
                        v0 = self.manifold.log(x0, x1)

                        # RCFM Step 2: Compute interpolated point on geodesic z_t = exp_{x0}(t * v0)
                        noisy_latents = self.manifold.exp(x0, t_reshaped * v0)

                        # RCFM Step 3: Compute target velocity u_t via parallel transport of v0
                        # For the Euclidean case, u_t is just v0 itself. For other manifolds, this would change.
                        target = self.manifold.parallel_transport(x0, x1, t_reshaped, v0)

                    # --- Fallback to original logic if Riemannian Flow is disabled ---
                    else:
                        ...
                        if int(bsz) != int(self.config.train_batch_size):
                            logger.error(f"Received {bsz} latents, but expected {self.config.train_batch_size}. Processing short batch.")
                        ...
                        if self.config.input_perturbation != 0 and (not self.config.input_perturbation_steps or self.state["global_step"] < self.config.input_perturbation_steps):
                            input_perturbation = self.config.input_perturbation
                            if self.config.input_perturbation_steps:
                                input_perturbation *= 1.0 - (self.state["global_step"] / self.config.input_perturbation_steps)
                        ...
                        if self.config.flow_matching:
                            ...
                            noisy_latents = self.noise_scheduler.add_noise(latents.float(), input_noise.float(), timesteps).to(device=self.accelerator.device, dtype=self.config.weight_dtype)
                    ...
                    # --- RCFM Loss Calculation ---
                    if getattr(self.config, 'riemannian_flow', True):
                        # The RCFM objective is the squared norm of the difference in the tangent space.
                        # For the Euclidean manifold, this corresponds to a simple MSE loss.
                        # Note: This simplifies the loss, removing the min-SNR gamma logic,
                        # as the RCFM paper specifies a direct metric-based loss.
                        training_logger.debug("Calculating loss using Riemannian metric (Euclidean MSE).")
                    ...
                    # --- Original Loss Calculation ---
                        ...
                        if self.config.flow_matching:
                            ...
                            snr_divisor = snr
                            if (self.noise_scheduler.config.prediction_type == "v_prediction" or (self.config.flow_matching and self.config.flow_matching_loss == "diffusion")):
                                snr_divisor = snr + 1

                            training_logger.debug("Calculating MSE loss weights using SNR as divisor")
                            mse_loss_weights = (torch.stack([snr, self.config.snr_gamma * torch.ones_like(timesteps)], dim=1).min(dim=1)[0] / snr_divisor)
                        ...
                        if conditioning_type == "mask":
                            mask_image = (batch["conditioning_pixel_values"].to(dtype=loss.dtype, device=loss.device)[:, 0].unsqueeze(1))
