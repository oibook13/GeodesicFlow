            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        ...
                        if hasattr(self.config, 'riemannian_flow_matching') and self.config.riemannian_flow_matching:
                            # Riemannian Rectified Flow via Ambient Space Projection
                            manifold_type = getattr(self.config, 'riemannian_manifold_type', 'sphere')

                            if manifold_type == 'sphere':
                                # 1. Interpolate in ambient space: z'_t = (1-t)x_0 + t*epsilon
                                z_prime_t = (1 - sigmas) * latents + sigmas * input_noise
                                original_shape = z_prime_t.shape

                                # 2. Project onto manifold: z_t = proj(z'_t)
                                # For the sphere, this is normalization to unit length.
                                z_t = F.normalize(z_prime_t.view(original_shape[0], -1), p=2, dim=1).view(original_shape)
                                noisy_latents = z_t # This is the input to the model

                                # 3. Calculate target velocity in the tangent space
                                # Ambient velocity: v = epsilon - x_0
                                ambient_velocity = noise - latents
                                # Project ambient velocity onto the normal vector z_t: (v^T z_t) z_t
                                dot_product = torch.sum(ambient_velocity * z_t, dim=tuple(range(1, z_t.ndim)), keepdim=True)
                                # The target is the tangent component: u_t = v - (v^T z_t) z_t
                                target = ambient_velocity - dot_product * z_t
                            else:
                                raise NotImplementedError(f"Riemannian flow matching for manifold type '{manifold_type}' is not implemented.")
                        else:
