...
class Trainer:
    ...
    def parse_arguments(self, args=None, disable_accelerator: bool = False):
        ...
        # setattr(self.config, 'geodesicflow_metric_lr', getattr(self.config, 'geodesicflow_metric_lr', 1e-5))
        setattr(self.config, 'geodesicflow_metric_lr', getattr(self.config, 'geodesicflow_metric_lr', self.config.learning_rate))
        setattr(self.config, 'geodesicflow_metric_reg', getattr(self.config, 'geodesicflow_metric_reg', 1e-4))
            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.geodesicflow_enabled:
                        ...
                        # The metric is a function of the interpolated point z_t (noisy_latents) and conditioning
                        log_diag_g = self.metric_network(
                            noisy_latents,
                            timesteps,
                            encoder_hidden_states,
                            added_cond_kwargs=added_cond_kwargs,
                        ...
                        diag_g = torch.exp(log_diag_g) + self.config.geodesicflow_metric_reg
                        
                        # Loss is the squared norm w.r.t the metric g, averaged over all elements
                        loss = ((residual.float() ** 2) / diag_g.float()).mean()
