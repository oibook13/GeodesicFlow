            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        ...
                        # Check if RFGG (Rectified Flow with Geodesic Guidance) is enabled
                        if getattr(self.config, 'riemannian_geodesic_guidance_enabled', True):
                            # ------------------------------------------------------------------
                            # --- Rectified Flow with Geodesic Guidance (RFGG) START ---
                            # ------------------------------------------------------------------
                            
                            # 1. Get Hyperparameters from config
                            TRACTABILITY_THRESHOLD = getattr(self.config, 'riemannian_tractability_threshold', 2.98) # Default: ~0.95*pi
                            ...
                            # Reshape latents for manifold operations
                            original_shape = latents.shape
                            x0_flat = latents.view(bsz, -1)
                            eps_flat = input_noise.view(bsz, -1)
                            t = sigmas.view(-1, 1)

                            # CRITICAL FIX: Ensure initial points are on the manifold (sphere) to prevent NaN errors.
                            x0 = F.normalize(x0_flat, p=2, dim=1)
                            eps = F.normalize(eps_flat, p=2, dim=1)

                            # --- A. UNIFIED AND STABLE PATH FORMULATION (Always Projection) ---
                            # Linear interpolation in ambient space
                            z_prime_t = (1 - t) * x0 + t * eps
                            # Handle potential singularities near the origin
                            ...
                            # Project onto the manifold to get the path point z_t
                            zt_flat = F.normalize(z_prime_t, p=2, dim=1)
                            noisy_latents = zt_flat.view(original_shape)

                            # --- B. GEODESIC GUIDANCE for an OPTIMAL VELOCITY TARGET ---
                            # 2. Determine the optimal geodesic guidance vector (v_g) at the start of the path (T_{x_0}M)
                            ...
                            # Decide whether to use logmap based on tractability threshold (a proxy for T(x0, eps))
                            use_logmap_mask = (geodesic_dist > TRACTABILITY_THRESHOLD)

                            # Case 1: Geodesic is tractable -> use logmap for v_g
                            theta = geodesic_dist
                            v_g_logmap = (theta / torch.sin(theta).clamp(min=EPS)) * (eps - torch.cos(theta) * x0)

                            # Case 2: Fallback -> project ambient direction onto the start tangent space T_{x_0}M
                            ambient_velocity_at_x0 = eps - x0
                            # Projection: v_proj = v - dot(v, n) * n, where n=x0 for the sphere
                            v_g_fallback = ambient_velocity_at_x0 - torch.sum(ambient_velocity_at_x0 * x0, dim=1, keepdim=True) * x0

                            # Combine to get the final guidance vector v_g using the mask
                            v_g = torch.where(use_logmap_mask, v_g_logmap, v_g_fallback)

                            # 3. Compute the final target velocity (u_t) at the current path point (z_t)
                            # Project the single, optimal guidance vector v_g onto the local tangent space T_{z_t}M
                            ut_flat = v_g - torch.sum(v_g * zt_flat, dim=1, keepdim=True) * zt_flat
                            target = ut_flat.view(original_shape)

                            # Ensure no regularization loss is used, as it's not part of the RFGG formulation
                            reg_loss = None

                            # ----------------------------------------------------------------
                            # --- Rectified Flow with Geodesic Guidance (RFGG) END ---
                            # ----------------------------------------------------------------
                    ...
                    if self.config.flow_matching:
                        # For RFGG and Euclidean Flow, the loss is a simple MSE flow-matching term
                        ...
                        # The regularization term 'reg_loss' is None for RFGG, so this correctly defaults to the simple matching loss.
