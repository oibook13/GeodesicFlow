...
class SaveHookManager_proposed:
    def __init__(
        self,
        args,
        unet,
        ...
        text_encoder_1,
        text_encoder_2,
        use_deepspeed_optimizer: bool = False,
        metric_network=None,  # <-- 1. MODIFIED: Accept the new argument
    ):
        self.args = args
        self.unet = unet
        self.transformer = transformer
        ...
        self.text_encoder_1 = text_encoder_1
        self.text_encoder_2 = text_encoder_2
        self.metric_network = metric_network  # <-- 2. ADDED: Store the metric network
        self.use_deepspeed_optimizer = use_deepspeed_optimizer
        
        # This part seems to be from a newer version of the hook, keeping it
        self.denoiser_class = None
        self.denoiser_subdir = None
        self.pipeline_class = None
        if self.unet is not None:
    ...
    def save_model_hook(self, models, weights, output_dir):
        ...
        # Handle saving the metric network separately
        if self.args.geodesicflow_enabled and self.metric_network is not None:
            logger.info("Saving GeodesicFlow Metric Network state.")
            metric_network_state_dict = unwrap_model(self.accelerator, self.metric_network).state_dict()
            save_file(
                metric_network_state_dict,
                os.path.join(output_dir, "metric_network.safetensors"),
            )
            # Remove the metric network from the models list to prevent the accelerator from saving it
            for i, model in enumerate(models):
                if model == self.metric_network:
                    models.pop(i)
                    if weights:
                        weights.pop(i)
                    break
    ...
    def load_model_hook(self, models, input_dir):
        ...
        if os.path.exists(training_state_path):
            ...
            logger.warning(f"Could not find training state file at {training_state_path}")
        
        # Handle loading the metric network separately
        if self.args.geodesicflow_enabled and self.metric_network is not None:
            metric_network_path = os.path.join(input_dir, "metric_network.safetensors")
            if os.path.exists(metric_network_path):
                logger.info("Loading GeodesicFlow Metric Network state from checkpoint.")
                state_dict = load_file(metric_network_path)
                unwrap_model(self.accelerator, self.metric_network).load_state_dict(state_dict)
                # Remove metric network from models list so accelerator doesn't try to load it
                for i, model in enumerate(models):
                    if model == self.metric_network:
                        models.pop(i)
                        break
            else:
                logger.warning("Metric network state not found in checkpoint. Skipping.")
        
        # Continue with original load logic
        if "lora" in self.args.model_type and self.args.lora_type == "standard":
            self._load_lora(models=models, input_dir=input_dir)
        elif "lora" in self.args.model_type and self.args.lora_type == "lycoris":
            self._load_lycoris(models=models, input_dir=input_dir)
