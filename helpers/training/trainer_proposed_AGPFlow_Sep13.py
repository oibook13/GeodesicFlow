...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.state["first_epoch"], self.config.num_train_epochs + 1):
            ...
            while True:
                ...
                with self.accelerator.accumulate(training_models):
                    ...
                    if self.config.flow_matching:
                        # --- This block handles ALL Flow Matching logic (Euclidean and Riemannian) ---
                        
                        # Sample sigmas (timesteps for flow matching)
                        if (
                        ...
                        for t_step in timesteps.tolist():
                            self.timesteps_buffer.append((self.state["global_step"], t_step))
                        
                        input_noise = noise

                        # Check if AGP-Flow is enabled
                        if getattr(self.config, 'riemannian_adaptive_path_enabled', True):
                            # ------------------------------------------------------------------
                            # --- Adaptive Geodesic-Projection Rectified Flow (AGP-Flow) START ---
                            # ------------------------------------------------------------------
                            
                            # Hyperparameters from config
                            DIST_THRESHOLD = getattr(self.config, 'riemannian_distance_threshold', 2.8) # Default: ~0.9*pi
                            REG_WEIGHT = getattr(self.config, 'riemannian_regularization_weight', 0.01)
                            NOISE_SCALE = getattr(self.config, 'riemannian_singularity_noise_scale', 1e-4)
                            SINGULARITY_THRESHOLD = getattr(self.config, 'riemannian_singularity_threshold', 1e-5)
                            EPS = 1e-6 # Small value for numerical stability

                            original_shape = latents.shape
                            
                            # --- IMPROVEMENT: Project inputs onto the sphere FIRST ---
                            # This is the critical fix to ensure inputs are valid manifold points
                            # and prevent loss explosion.
                            latents_on_sphere = F.normalize(latents.view(bsz, -1), p=2, dim=1).view(original_shape)
                            noise_on_sphere = F.normalize(input_noise.view(bsz, -1), p=2, dim=1).view(original_shape)
                            # --- END IMPROVEMENT ---

                            # Reshape for manifold operations: (B, C, H, W) -> (B, D)
                            x0 = latents_on_sphere.reshape(bsz, -1)
                            eps = noise_on_sphere.reshape(bsz, -1)
                            t = sigmas.view(-1, 1)

                            # --- Sphere Manifold Operations (Vectorized) ---
                            # Geodesic distance: d(x, y) = arccos(dot(x, y))
                            dot_product = torch.sum(x0 * eps, dim=1, keepdim=True)
                            geodesic_dist = torch.acos(torch.clamp(dot_product, -1.0 + EPS, 1.0 - EPS))

                            # Adaptive path selection mask
                            use_geodesic_mask = (geodesic_dist > DIST_THRESHOLD).squeeze(-1)
                            
                            # --- Path 1: Geodesic Path (for samples where use_geodesic_mask is True) ---
                            theta = geodesic_dist
                            v_initial = (theta / torch.sin(theta).clamp(min=EPS)) * (eps - torch.cos(theta) * x0)
                            v_norm = torch.norm(v_initial, p=2, dim=1, keepdim=True).clamp(min=EPS)
                            zt_geo = torch.cos(t * v_norm) * x0 + torch.sin(t * v_norm) * (v_initial / v_norm)
                            ut_geo = -v_norm * torch.sin(t * v_norm) * x0 + v_norm * torch.cos(t * v_norm) * (v_initial / v_norm)

                            # --- Path 2: Projection Path (for samples where use_geodesic_mask is False) ---
                            z_prime_t = (1 - t) * x0 + t * eps
                            z_prime_norm = torch.norm(z_prime_t, p=2, dim=1, keepdim=True)
                            is_singular = (z_prime_norm < SINGULARITY_THRESHOLD)
                            if torch.any(is_singular):
                                z_prime_t += is_singular * torch.randn_like(z_prime_t) * NOISE_SCALE
                            zt_proj = F.normalize(z_prime_t, p=2, dim=1)
                            ambient_velocity = eps - x0
                            ut_proj = ambient_velocity - torch.sum(ambient_velocity * zt_proj, dim=1, keepdim=True) * zt_proj

                            # --- Combine paths and velocities based on the mask ---
                            noisy_latents_flat = torch.where(use_geodesic_mask.unsqueeze(-1), zt_geo, zt_proj)
                            target_flat = torch.where(use_geodesic_mask.unsqueeze(-1), ut_geo, ut_proj)
                            
                            # Reshape back to original tensor shape
                            noisy_latents = noisy_latents_flat.view(original_shape)
                            target = target_flat.view(original_shape)

                            # --- Calculate Regularization Loss ---
                            reg_mask = ~use_geodesic_mask
                            reg_loss = torch.zeros(bsz, device=self.accelerator.device)
                            if torch.any(reg_mask) and REG_WEIGHT > 0:
                                # For regularization, we need the true geodesic point z_geo
                                z_geo_for_reg = torch.cos(t * v_norm) * x0 + torch.sin(t * v_norm) * (v_initial / v_norm)
                                # We compute the squared geodesic distance between the projected point and the true geodesic point
                                reg_dot_product = torch.sum(zt_proj * z_geo_for_reg, dim=1)
                                reg_dist_sq = torch.acos(torch.clamp(reg_dot_product, -1.0 + EPS, 1.0 - EPS))**2
                                reg_loss = torch.where(
                                    reg_mask,
                                    REG_WEIGHT * reg_dist_sq,
                                    torch.zeros_like(reg_loss)
                                )
                            # ----------------------------------------------------------------
                            # --- Adaptive Geodesic-Projection Rectified Flow (AGP-Flow) END ---
                            # ----------------------------------------------------------------
                            ...
                            reg_loss = None # No regularization in Euclidean case
                            ...
                            if self.config.flow_matching_loss == "diffusers":
                                ...
                                target = (noisy_latents - latents) / sigmas.view(-1, 1, 1, 1)

                    else:
                        # --- This block handles ALL Diffusion logic ---
                        reg_loss = None # No regularization in diffusion case
                        if self.config.offset_noise:
                             if (self.config.noise_offset_probability == 1.0 or random.random() < self.config.noise_offset_probability):
                                noise = noise + self.config.noise_offset * torch.randn(latents.shape[0], latents.shape[1], 1, 1, device=latents.device)
                    ...
                    if (
                        ...
                        and self.noise_scheduler.config.get("prediction_type") == "sample"
                    ):
                    ...
                    if self.config.flow_matching:
                        # AGP-Flow and Euclidean Flow both use MSE for the matching term
                        match_loss = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                        match_loss = match_loss.mean(dim=list(range(1, len(match_loss.shape))))
                        
                        # Add regularization term if it exists (from AGP-Flow)
                        if reg_loss is not None:
                           loss = (match_loss + reg_loss).mean()
                        else:
                           loss = match_loss.mean()
                        ...
                        snr_divisor = snr
                        if (self.noise_scheduler.config.get("prediction_type") == "v_prediction"):
                            snr_divisor = snr + 1
                    ...
                    if not self.config.disable_accelerator:
                        ...
                        if self.config.optimizer_release_gradients:
                            ...
                            should_not_release_gradients = (
                                (step + step_offset) % self.config.gradient_accumulation_steps != 0
                            )
                ...
                if self.accelerator.sync_gradients:
                    try:
                        ...
                        logger.error(f"Failed to get learning rate: {e}")
                    ...
                    if self.config.report_to == "wandb" and self.accelerator.is_main_process:
                        data = [[it, ts] for it, ts in self.timesteps_buffer]
                        table = wandb.Table(data=data, columns=["global_step", "timestep"])
                        wandb_logs["timesteps_scatter"] = wandb.plot.scatter(
                            table, "global_step", "timestep", title="Timestep distribution"
                        )
                    self.timesteps_buffer = []

                    if training_luminance_values:
                         avg_luminance = sum(training_luminance_values) / len(training_luminance_values)
                         wandb_logs["train_luminance"] = avg_luminance
                    
                    self.accelerator.log(wandb_logs, step=self.state["global_step"])
                    training_luminance_values = []
                    self.train_loss = 0.0

                    if (self.config.webhook_reporting_interval is not None and self.state["global_step"] % self.config.webhook_reporting_interval == 0):
                        structured_data = {
                            "state": self.state,
                    ...
                    if self.state["global_step"] % self.config.checkpointing_steps == 0:
                            ...
                            if self.config.checkpoints_total_limit is not None:
                                ...
                                if len(checkpoints) >= self.config.checkpoints_total_limit:
                                    ...
                                    for ckpt in checkpoints[:num_to_remove]:
                                        shutil.rmtree(os.path.join(self.config.output_dir, ckpt))
                        ...
                        if (self.accelerator.is_main_process or self.config.use_deepspeed_optimizer):
                            ...
                            for _, backend in StateTracker.get_data_backends().items():
                                if "sampler" in backend:
                                    backend["sampler"].save_state(state_path=os.path.join(save_path, self.model_hooks.training_state_path))

                    if (self.config.accelerator_cache_clear_interval is not None and self.state["global_step"] % self.config.accelerator_cache_clear_interval == 0):
                        reclaim_memory()
                    ...
                    if self.accelerator.is_main_process:
                        try:
                            self.hub_manager.upload_latest_checkpoint(
                                validation_images=(getattr(self.validation, "validation_images", None)),
            ...
            if (self.state["global_step"] >= self.config.max_train_steps or epoch > self.config.num_train_epochs):
                logger.info(f"Exiting training loop at epoch {epoch}, step {self.state['global_step']}")
                break
        ...
        if self.accelerator.is_main_process:
            ...
            if "lora" in self.config.model_type and "standard" == self.config.lora_type.lower():
                if self.transformer is not None:
                    ...
                    unet_lora_layers = convert_state_dict_to_diffusers(get_peft_model_state_dict(self.unet))
                else:
                    raise Exception("Couldn't locate model for LoRA export.")

                if self.config.train_text_encoder:
                    self.text_encoder_1 = self.accelerator.unwrap_model(self.text_encoder_1)
                    text_encoder_lora_layers = convert_state_dict_to_diffusers(get_peft_model_state_dict(self.text_encoder_1))
                    if self.text_encoder_2 is not None:
                ...
                if self.config.model_family == "flux":
                    ...
                    FluxPipeline.save_lora_weights(save_directory=self.config.output_dir, transformer_lora_layers=transformer_lora_layers, text_encoder_lora_layers=text_encoder_lora_layers)
                elif self.config.model_family == "sd3":
                    StableDiffusion3Pipeline.save_lora_weights(save_directory=self.config.output_dir, transformer_lora_layers=transformer_lora_layers, text_encoder_lora_layers=text_encoder_lora_layers, text_encoder_2_lora_layers=text_encoder_2_lora_layers)
                else:
                    StableDiffusionXLPipeline.save_lora_weights(save_directory=self.config.output_dir, unet_lora_layers=unet_lora_layers, text_encoder_lora_layers=text_encoder_lora_layers, text_encoder_2_lora_layers=text_encoder_2_lora_layers)
                
                del self.unet, self.transformer, text_encoder_lora_layers, text_encoder_2_lora_layers
                reclaim_memory()
                ...
                if (self.accelerator.is_main_process or self.config.use_deepspeed_optimizer):
                    ...
                    if getattr(self.accelerator, "_lycoris_wrapped_network", None) is not None:
                        ...
                        shutil.copy2(self.config.lycoris_config, os.path.join(self.config.output_dir, "lycoris_config.json"))
            ...
            if self.config.model_type == "full":
                if self.config.model_family == "sd3":
                    self.pipeline = StableDiffusion3Pipeline.from_pretrained(
                        self.config.pretrained_model_name_or_path,
                        text_encoder=self.text_encoder_1 or (self.text_encoder_cls_1.from_pretrained(self.config.pretrained_model_name_or_path, subfolder="text_encoder", revision=self.config.revision, variant=self.config.variant) if self.config.save_text_encoder else None),
                        tokenizer=self.tokenizer_1,
                        text_encoder_2=self.text_encoder_2 or (self.text_encoder_cls_2.from_pretrained(self.config.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=self.config.revision, variant=self.config.variant) if self.config.save_text_encoder else None),
                        tokenizer_2=self.tokenizer_2,
                        text_encoder_3=self.text_encoder_3 or (self.text_encoder_cls_3.from_pretrained(self.config.pretrained_model_name_or_path, subfolder="text_encoder_3", revision=self.config.revision, variant=self.config.variant) if self.config.save_text_encoder else None),
                        tokenizer_3=self.tokenizer_3,
                        vae=self.vae or AutoencoderKL.from_pretrained(self.config.vae_path, subfolder=("vae" if self.config.pretrained_vae_model_name_or_path is None else None), revision=self.config.revision, variant=self.config.variant, force_upcast=False),
                        transformer=self.transformer,
                    )
                    ...
                    self.pipeline = sdxl_pipeline_cls.from_pretrained(
                        self.config.pretrained_model_name_or_path,
                        text_encoder=(self.text_encoder_cls_1.from_pretrained(self.config.pretrained_model_name_or_path, subfolder="text_encoder", revision=self.config.revision, variant=self.config.variant) if self.config.save_text_encoder else None),
                        text_encoder_2=(self.text_encoder_cls_2.from_pretrained(self.config.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=self.config.revision, variant=self.config.variant) if self.config.save_text_encoder else None),
                        tokenizer=self.tokenizer_1,
                        tokenizer_2=self.tokenizer_2,
                        vae=StateTracker.get_vae() or AutoencoderKL.from_pretrained(self.config.vae_path, subfolder=("vae" if self.config.pretrained_vae_model_name_or_path is None else None), revision=self.config.revision, variant=self.config.variant, force_upcast=False),
                        unet=self.unet,
                        revision=self.config.revision,
                ...
                if not self.config.flow_matching and self.config.validation_noise_scheduler is not None:
                    self.pipeline.scheduler = SCHEDULER_NAME_MAP[self.config.validation_noise_scheduler].from_pretrained(
                        self.config.pretrained_model_name_or_path,
                        revision=self.config.revision,
                        subfolder="scheduler",
