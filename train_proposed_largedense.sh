...
if [ -f "${ACCELERATE_CONFIG_PATH}" ]; then
    ...
    accelerate launch --config_file="${ACCELERATE_CONFIG_PATH}" --main_process_port="${MAIN_PROCESS_PORT}" train_proposed_largedense.py
else
    echo "Accelerate config file not found: ${ACCELERATE_CONFIG_PATH}. Using values from config.env."
    accelerate launch ${ACCELERATE_EXTRA_ARGS} --main_process_port="${MAIN_PROCESS_PORT}" --mixed_precision="${MIXED_PRECISION}" --num_processes="${TRAINING_NUM_PROCESSES}" --num_machines="${TRAINING_NUM_MACHINES}" --dynamo_backend="${TRAINING_DYNAMO_BACKEND}" train_proposed_largedense.py
