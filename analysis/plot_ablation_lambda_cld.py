...
# --- Primary Y-Axis (FID) ---
ax1.set_xlabel('λ', fontsize=12, fontweight='normal')
ax1.set_ylabel('FID score', color=color_fid, fontsize=12, fontweight='normal')
...
ax1.tick_params(axis='y', labelcolor=color_fid, labelsize=10, direction='out', length=4)
ax1.tick_params(axis='x', labelsize=10, direction='out', length=4)
ax1.set_xscale('log')

# Set y-axis limits with some padding
ax1.set_ylim(20.0, 22.0)
...
ax2.set_ylabel('METEOR score', color=color_meteor, fontsize=12, fontweight='normal')
...
ax2.tick_params(axis='y', labelcolor=color_meteor, labelsize=10, direction='out', length=4)
ax2.set_ylim(29.5, 30.5)
ax2.yaxis.set_minor_locator(plt.MultipleLocator(0.1))

# --- Grid ---
ax1.grid(True, which='major', axis='both', alpha=0.3, linewidth=0.5, linestyle='-')
ax1.grid(True, which='minor', axis='both', alpha=0.15, linewidth=0.3, linestyle='-')
ax1.set_axisbelow(True)

# --- Legend ---
# Create custom legend with clean styling
legend_elements = [
    plt.Line2D([0], [0], color=color_fid, marker='o', markersize=6, 
               markerfacecolor=color_fid, markeredgecolor='white', 
               markeredgewidth=1.2, linestyle='-', linewidth=1.8, label='FID'),
    plt.Line2D([0], [0], color=color_meteor, marker='s', markersize=5, 
               markerfacecolor=color_meteor, markeredgecolor='white', 
               markeredgewidth=1.2, linestyle='--', linewidth=1.8, 
               dashes=(5, 3), label='METEOR')
]
...
ax1.set_xticklabels([f'{val:.4f}' for val in sorted_lambdas])
