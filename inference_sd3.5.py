...
def main():
    ...
    # checkpoints = [int(x.split("-")[1]) for x in os.listdir(args.model_path) if x.startswith("checkpoint-")]
    # checkpoints.sort()

    # args.model_path = os.path.join(args.model_path, 'checkpoint-' + str(checkpoints[-1]))
    # args.output_dir = os.path.join(args.output_dir, args.model_path.split('/', 1)[1], args.split)
    ...
    for batch in dataloader:
        ...
        if args.num_prompts_per_run == 1:
            ...
            for prompts, prompt_ids in zip(prompts_per_run, prompt_ids_per_run):
                # if all of them exists, skip, otherwise generate
