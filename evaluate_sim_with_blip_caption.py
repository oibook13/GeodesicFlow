...
def calculate_metrics(generated_captions, ground_truth_captions):
    ...
    return metrics, meteor_scores, [' '.join(h) for h in hypotheses], [' '.join(r[0]) for r in references]

def calculate_individual_metrics(hyp, ref):
    """Calculate individual BLEU-4, METEOR, and ROUGE-L scores for a single caption pair"""
    # Prepare for BLEU calculation
    reference = [ref.split()]
    ...
    smooth = SmoothingFunction().method1
    bleu4 = corpus_bleu([reference], [hypothesis], weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=smooth)
    
    # Calculate METEOR score
    meteor_score_val = meteor_score(reference, hypothesis)
    ...
    return {
        'BLEU-4': bleu4 * 100,
        'METEOR': meteor_score_val * 100,
        'ROUGE-L': rouge_scores[0]['rouge-l']['f'] * 100
    }
...
def evaluate_captions(image_folder, caption_folder, output_file=None, model_name="Salesforce/blip-image-captioning-base"):
    ...
    for img_file in tqdm(image_files):
        ...
        if generated and ground_truth:
            ...
            # Calculate individual metrics for this image
            individual_metrics = calculate_individual_metrics(generated, ground_truth)
            
            # Store results with individual metrics
            results_dict[img_id] = {
                'generated': generated,
                'ground_truth': ground_truth,
                'BLEU-4': individual_metrics['BLEU-4'],
                'METEOR': individual_metrics['METEOR'],
                'ROUGE-L': individual_metrics['ROUGE-L']
            }
    
    # Calculate metrics
    if generated_captions and ground_truth_captions:
        metrics, _, _, _ = calculate_metrics(generated_captions, ground_truth_captions)
